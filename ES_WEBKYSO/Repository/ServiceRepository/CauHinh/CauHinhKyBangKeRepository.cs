...
{
    public class LOAI_BANGKE_DONVIRepository<T> : BaseRepository<LOAI_BANGKE_DONVI>
    {
        public LOAI_BANGKE_DONVIRepository(DbContext context, GenericRepository repo) : base(context, repo)
        {
        }
        ...
        public override List<LOAI_BANGKE_DONVI> ManagerGetAllForIndex(FindModelGcs findModelGcs, string orderKey, ref Paging page)
        {
            if (orderKey.EndsWith("String"))
            {
                orderKey = orderKey.Replace("String", "");
