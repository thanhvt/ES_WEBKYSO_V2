...
{
    public class DM_DOIGCSRepository<T> : BaseRepository<DM_DOIGCS>
    {
        public DM_DOIGCSRepository(DbContext context, GenericRepository repo) : base(context, repo)
        {
        }
        public override List<DM_DOIGCS> GETALL()
        {
            try
            {
        ...
        public override List<DM_DOIGCS> ManagerGetAllForIndex(FindModelGcs findModel, string searchString)
        {
            var ret = new List<DM_DOIGCS>();
            ret = GetAll(o => (findModel.MaDoi == o.MA_DOIGCS || searchString == o.MA_DOIGCS))
                .ToList()
                .Select(x => new DM_DOIGCS
                {
                    ID = x.ID,
